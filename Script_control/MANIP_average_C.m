...
function SC_FC_C(gender,N_roi, scan_dir)
        N_window=9;
        ...
        switch gender
            ...
            case 1
                for n=1:N_res 
                    load(['../','results_SC_strength_FC_F','/','roi_',num2str(N_roi),'_1_',scan_dir,'/','LH/',num2str(resolution(n)),'/SC_FC_results'])
                    c_group_LH{n}=c_SC_FC;
                end
                
                for n=1:N_res
                    load(['../','results_SC_strength_FC_F','/','roi_',num2str(N_roi),'_1_',scan_dir,'/','RH/',num2str(resolution(n)),'/SC_FC_results'])
                    c_group_RH{n}=c_SC_FC;
                end
            otherwise
                for n=1:N_res 
                    load(['../','results_SC_strength_FC_M','/','roi_',num2str(N_roi),'_1_',scan_dir,'/','LH/',num2str(resolution(n)),'/SC_FC_results'])
                    c_group_LH{n}=c_SC_FC;
                end
                
                for n=1:N_res
                    load(['../','results_SC_strength_FC_M','/','roi_',num2str(N_roi),'_1_',scan_dir,'/','RH/',num2str(resolution(n)),'/SC_FC_results'])
                    c_group_RH{n}=c_SC_FC;
                end
        end
        ...
        c_tempora=zeros(N_res,1); % for storage purpose
        
        % calculate the mean of c across resolution
        for j=1:N_window
            for i=1:N_roi/2
                for n=1:N_res
                   c_tempora(n)=c_group_LH{n}(i,j);
                end
                c_mean(i,j)=mean(c_tempora);
                c_tempora=zeros(N_res,1);
            end
        end
        ...
        for j=1:N_window
            for i=1:N_roi/2
                for n=1:N_res
                   c_tempora(n)=c_group_RH{n}(i,j);
                end
                c_mean(N_roi/2+i,j)=mean(c_tempora);
                c_tempora=zeros(N_res,1);
            end
        end
        ...
        %            case 1
        %                if type==1
        %                    saveas(gcf,['../','results_SC_strength_FC_F','/','roi_',num2str(N_roi),'_1_',scan_dir,'/C_mean_SC_FC_',scan_dir,'_',num2str(j),'.fig'])
        %                else 
        %                    saveas(gcf,['../','results_SC_count_FC_F','/','roi_',num2str(N_roi),'_1_',scan_dir,'/C_mean_SC_FC_',scan_dir,'_',num2str(j),'.fig'])     
        %                end
        %            otherwise
        %                if type==1
        %                    saveas(gcf,['../','results_SC_strength_FC_M','/','roi_',num2str(N_roi),'_1_',scan_dir,'/C_mean_SC_FC_',scan_dir,'_',num2str(j),'.fig'])
        %                else 
        %                    saveas(gcf,['../','results_SC_count_FC_M','/','roi_',num2str(N_roi),'_1_',scan_dir,'/C_mean_SC_FC_',scan_dir,'_',num2str(j),'.fig'])     
        %                end
        %        end
        % end
        ...
        switch gender
            ...
            case 1
                if type==1  
                    SC_FC_results_path=fullfile(data_path,['../','results_SC_strength_FC_F','/','roi_',num2str(N_roi),'_1_',scan_dir,'/c_mean_SC_FC_results']);
                else 
                    SC_FC_results_path=fullfile(data_path,['../','results_SC_count_FC_F','/','roi_',num2str(N_roi),'_1_',scan_dir,'/c_mean_SC_FC_results']);
                end
            otherwise
                if type==1  
                    SC_FC_results_path=fullfile(data_path,['../','results_SC_strength_FC_M','/','roi_',num2str(N_roi),'_1_',scan_dir,'/c_mean_SC_FC_results']);
                else 
                    SC_FC_results_path=fullfile(data_path,['../','results_SC_count_FC_M','/','roi_',num2str(N_roi),'_1_',scan_dir,'/c_mean_SC_FC_results']);
                end
        end
        ...
        %     case 1
        %         saveas(gcf,['../','results_SC_strength_FC_F','/','roi_',num2str(N_roi),'_1_',scan_dir,'/LH','/C_mean_SC_FC_7_network_LH',scan_dir,'.fig'])
        %     otherwise
        %         saveas(gcf,['../','results_SC_strength_FC_M','/','roi_',num2str(N_roi),'_1_',scan_dir,'/LH','/C_mean_SC_FC_7_network_LH',scan_dir,'.fig'])
        % end
        % 
        ...
        %     case 1
        %         saveas(gcf,['../','results_SC_strength_FC_F','/','roi_',num2str(N_roi),'_1_',scan_dir,'/RH','/C_mean_SC_FC_7_network_',scan_dir,'.fig'])
        %     otherwise
        %         saveas(gcf,['../','results_SC_strength_FC_M','/','roi_',num2str(N_roi),'_1_',scan_dir,'/RH','/C_mean_SC_FC_7_network_',scan_dir,'.fig'])
        % end
        ...
        colors = [0 0.447 0.741;      % Vis - blue
                  0.850 0.325 0.098;  % SomMot - orange
                  0.929 0.694 0.125;  % DorsAttn - yellow
                  0.494 0.184 0.556;  % SalVentAttn - purple
                  0.466 0.674 0.188;  % Limbic - green
                  0.301 0.745 0.933;  % Cont - light blue
                  0.635 0.078 0.184]; % Default - red
        
        plot(1:9,c_LH_Vis(1,:),'Color',colors(1,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(1,:)); hold on
        plot(1:9,c_LH_SomMot(1,:),'Color',colors(2,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(2,:)); hold on
        plot(1:9,c_LH_DorsAttn(1,:),'Color',colors(3,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(3,:)); hold on
        plot(1:9,c_LH_SalVentAttn(1,:),'Color',colors(4,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(4,:)); hold on
        plot(1:9,c_LH_Limbic(1,:),'Color',colors(5,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(5,:)); hold on
        plot(1:9,c_LH_Cont(1,:),'Color',colors(6,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(6,:)); hold on
        plot(1:9,c_LH_Default(1,:),'Color',colors(7,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(7,:)); hold on
        
        legend({'LH Vis','LH SomMot','LH DorsAttn','LH SalVentAttn','LH Limbic','LH Cont','LH Default'}, ...
            'fontsize',12,'location','southwest')
        
        title({['ROI ',num2str(N_roi),' LH']},'fontsize',16)
        set(gca,'box','on','fontsize',16,'linewidth',1.2,'fontname','times')
        set(gca,'xticklabel',{'0-5','3-8','6-11','9-14','12-17','15-23','18-29','24-36','>36'},'FontSize',12)
        xlabel('Month','fontsize',16)
        ylabel('c','fontsize',16)
        xlim([1,9]); ylim([0,1])
        ...
        %% ====================== RH ======================
        figure
        plot(1:9,c_RH_Vis(1,:),'Color',colors(1,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(1,:)); hold on
        plot(1:9,c_RH_SomMot(1,:),'Color',colors(2,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(2,:)); hold on
        plot(1:9,c_RH_DorsAttn(1,:),'Color',colors(3,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(3,:)); hold on
        plot(1:9,c_RH_SalVentAttn(1,:),'Color',colors(4,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(4,:)); hold on
        plot(1:9,c_RH_Limbic(1,:),'Color',colors(5,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(5,:)); hold on
        plot(1:9,c_RH_Cont(1,:),'Color',colors(6,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(6,:)); hold on
        plot(1:9,c_RH_Default(1,:),'Color',colors(7,:), 'LineWidth',1.8, 'Marker','o','MarkerFaceColor',colors(7,:)); hold on
        
        legend({'RH Vis','RH SomMot','RH DorsAttn','RH SalVentAttn','RH Limbic','RH Cont','RH Default'}, ...
            'fontsize',12,'location','southwest')
